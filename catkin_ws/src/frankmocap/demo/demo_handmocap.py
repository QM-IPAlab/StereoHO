...
def run_hand_mocap(args, bbox_detector, hand_mocap, visualizer):
    ...
    if input_type == 'webcam':
        def callback1(data):
            cam1_image = cvBridge.imgmsg_to_cv2(data, 'bgr8')
            cam1_image_msg = np.asarray(cam1_image, dtype=np.int16).flatten()

        def callback2(data):
            cam2_image = cvBridge.imgmsg_to_cv2(data, 'bgr8')
            cam2_image_msg = np.asarray(cam2_image, dtype=np.int16).flatten()
        
        rospy.Subscriber("/camera1/color/image_raw", Image, callback1)
        rospy.Subscriber("/camera2/color/image_raw", Image, callback2)
