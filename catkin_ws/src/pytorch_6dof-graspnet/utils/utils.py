...
def convert_qt_to_grasps(qt):
    # Ts = qt[:, 4:]
    # Rs = quat2mat(qt[:, :4])
    # mat = torch.zeros((qt.shape[0], 4, 4), dtype=torch.float32)
    # mat[:, :3, :3] = Rs
    # mat[:, :3, 3] = Ts
    # mat[:, 3, 3] = 1
    rs, ts = convert_qt_to_rt(qt)
    rs = rs.unsqueeze(0).cpu().numpy()
    ts = ts.unsqueeze(0).cpu().numpy()
    selection_mask = np.ones((rs.shape[0], rs.shape[1]), dtype=np.bool)
    grasps = rot_and_trans_to_grasps(rs, ts, selection_mask)
