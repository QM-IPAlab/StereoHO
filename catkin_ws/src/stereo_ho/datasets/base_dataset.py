...
def CreateDataset(opt):
    ...
    elif opt.dataset_mode == 'obman':
        from datasets.obman_dataset import ObManDataset
        train_dataset = ObManDataset()
        test_dataset = ObManDataset()
        train_dataset.initialize(opt, 'train')
        test_dataset.initialize(opt, 'test')

    elif opt.dataset_mode == 'obman_code':
        from datasets.obman_dataset import ObManCodeDataset
        train_dataset = ObManCodeDataset()
        test_dataset = ObManCodeDataset()
        train_dataset.initialize(opt, 'train')
        test_dataset.initialize(opt, 'test')
    
    elif opt.dataset_mode == 'obman_img':
        from datasets.obman_dataset import ObManImgDataset
        train_dataset = ObManImgDataset()
        test_dataset = ObManImgDataset()
        train_dataset.initialize(opt, 'test')
        test_dataset.initialize(opt, 'test')
