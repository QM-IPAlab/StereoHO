...
def read_h5(sdf_h5_file, res=64):
...
def read_sdf(sdf_file, sdf_res=64):
    # sdf_res *= 4
    intsize = 4
    floatsize = 8
    ...
    sdf_value = sdf["value"][:-1, :-1, :-1]
    sdf_value = torch.Tensor(sdf_value).view(1, sdf_res, sdf_res, sdf_res)
    sdf_value = sdf_value.permute(0, 3, 2, 1)
    sdf_value = sdf_value[None, ...]
    return sdf_value
...
def ho_sdf_to_mesh(sdf_h, sdf_o, level=0.02, color=None, render_all=False, flip_faces=True):
    mesh_h = sdf_to_mesh(sdf_h, level=level, color=color, render_all=render_all, flip_faces=flip_faces)
    mesh_o = sdf_to_mesh(sdf_o, level=level, color=color, render_all=render_all, flip_faces=flip_faces)

    # combine mesh
    combined_mesh = pytorch3d.structures.join_meshes_as_scene([mesh_h, mesh_o])

    return combined_mesh

def sdf_to_mesh(sdf, level=0.02, color=None, render_all=False, flip_faces=True):
    # device='cuda'
    device=sdf.device
    ...
    for i in range(nimg_to_render):
        ...
        # smoothed_shape = mcubes.smooth(sdf_i)
        verts_i, faces_i = mcubes.marching_cubes(sdf_i, level)
        # verts_i = verts_i / n_cell - .5 
        verts_i = (verts_i / n_cell - .5) * 0.52
        bbox_min = np.array([-0.32, -0.36, -0.27])
        bbox_max = np.array([0.2,   0.16,  0.25])
        offset = (bbox_max + bbox_min) / 2
        verts_i = verts_i + offset
...
def render_mesh_with_pose(mesh_renderer, mesh, pose):
    print("render_mesh_with_pose")
    
    # render mesh with pose using trimesh
    # do not use pytorch3d
    # transform using trimesh
    verts = mesh.verts_list()
    faces = mesh.faces_list()
    textures = mesh.textures
    verts = np.array(verts[0].detach().cpu())
    faces = np.array(faces[0].detach().cpu())
    print("verts:", verts.shape)
    print("faces:", faces.shape)
    # construct mesh object in trimesh
    pose = pose[0].detach().cpu().numpy()
    print("pose:", pose)
    pose = np.linalg.inv(pose)
    print("pose:", pose.shape)
    print("pose:", pose)
    extra_rot = R.from_euler('yz', [-180,180], degrees=True).as_matrix()
    extra = np.eye(4)
    extra[:3, :3] = extra_rot
    pose = np.dot(pose, extra)

    # transform verts
    verts = np.dot(pose[:3, :3], verts.T).T + pose[:3, 3]

    mesh = trimesh.Trimesh(vertices=verts, faces=faces)
    scene = trimesh.Scene()
    scene.add_geometry(mesh)
    
    # put camera at origin
    # camera = trimesh.scene.Camera(resolution=(256, 256), focal=(10,10))#
    camera_transform = trimesh.transformations.translation_matrix([0, 0, 0])
    camera_transform[:3, :3] = np.eye(3)
    camera = scene.camera
    print("transform:", camera_transform)
    camera.transform = camera_transform
    print("transform:", camera_transform)
    print("camera K:", camera.K)
    cam_f = 150.
    camera.K = np.array([[cam_f, 0, 128], [0, cam_f, 128], [0, 0, 1]])
    print("camera K:", camera.K)
    scene.camera = camera

    data = scene.save_image(resolution=(256, 256), visible=True)
...
def load_mesh(obj_f, color=[1,1,1]):
    ...
    # verts_rgb = color.repeat(verts.shape[1], 1).unsqueeze(0)
